 ...
 * 通义千问平台策略实现
 */
class TongyiPlatform {
  constructor() {
    // Use the safeInherit helper to inherit from BasePlatform
    safeInherit(this, BasePlatform, 'TongyiPlatform');
   ...
   * @type {string}
   */
  static displayName = '通义千问';

  /**
   * 检查URL是否匹配通义千问
   * @param {string} url - 要检查的URL
   * @returns {boolean} - 如果URL匹配通义千问则返回true
   */
  ...
  extractTitle() {
    try {
      ...
      if (this.fallbackMode) {
        console.log('Using fallback mode for title extraction');
        return '通义千问对话';
      }
      ...
      if (titleElement && titleElement.textContent) {
        ...
        if (titleText && !titleText.includes('通义') && !titleText.includes('千问')) {
          return titleText;
        }
      }
      
      // 尝试从对话标题获取
      const chatTitle = document.querySelector('.conversation-title, .chat-title, .title, .session-name');
      if (chatTitle && chatTitle.textContent.trim()) {
        return chatTitle.textContent.trim();
      }
      
      // 查找第一条用户消息
      const firstUserMessage = document.querySelector('.user-message, .human-message, .question, .chat-message.human');
      if (firstUserMessage) {
        const text = firstUserMessage.textContent.trim();
        return text.length > 30 ? text.substring(0, 27) + '...' : text;
      }
    } catch (error) {
      console.error('Error extracting Tongyi title:', error);
    }
  ...
  async extractContext() {
    console.log('Extracting Tongyi context...');
    const messages = [];
    
    // Check if we're in fallback mode
    if (this.fallbackMode) {
      console.log('Using fallback mode for context extraction');
      return [{
        role: 'assistant',
    ...
    try {
      // 查找对话消息容器
      const messageContainers = document.querySelectorAll('.chat-item, .message-item, .message, .chat-message');
      console.log('Found Tongyi message containers:', messageContainers.length);
      ...
      messageContainers.forEach(container => {
        ...
        const isUser = container.classList.contains('user') || 
                      ...
                      container.hasAttribute('data-role-type') && container.getAttribute('data-role-type') === 'user';
        
        // 提取消息内容
        let content = '';
        const contentElement = container.querySelector('.content, .message-content, .text, .content-text');
        ...
        images.forEach(img => {
          if (img.src && !img.src.includes('data:image/svg+xml')) {
            ...
            console.log('Found image in Tongyi:', img.src);
          }
        });
        ...
        fileLinks.forEach(link => {
          if (link.href) {
            ...
            console.log('Found file in Tongyi:', link.href);
          }
        });
      ...
      console.error('Error extracting Tongyi context:', error);
    }
  ...
  async restoreContext(context, sourcePlatform) {
    console.log('Restoring context to Tongyi from', sourcePlatform, 'context length:', context.length);
    
    // Check if we're in fallback mode
    if (this.fallbackMode) {
      console.log('Cannot restore context: TongyiPlatform is in fallback mode');
      return false;
    }
    ...
    try {
      ...
      // 尝试创建新对话
      try {
        const newChatButton = document.querySelector('button.new-chat, button.create-chat, button[title*="新对话"]');
        if (newChatButton) {
      ...
      // 查找输入区域
      const textarea = document.querySelector('textarea, div[contenteditable="true"], .chat-input textarea, [placeholder*="发送"]');
      if (!textarea) {
        console.error('无法找到通义千问输入框，尝试更多选择器');
        ...
        if (possibleInputs.length === 0) {
          throw new Error('找不到通义千问输入框');
        }
        
        // 使用第一个找到的输入元素
        const success = await handleGenericInput(possibleInputs[0], this.formatContextForTongyi(context, sourcePlatform), sourcePlatform);
      ...
      console.log('Found Tongyi textarea:', textarea);
      const success = await handleGenericInput(textarea, this.formatContextForTongyi(context, sourcePlatform), sourcePlatform);
      ...
      console.error('Error restoring context to Tongyi:', error);
      return false;
    }
  }
   ...
   * @private
   */
  formatContextForTongyi(context, sourcePlatform) {
    // 准备恢复信息
    ...
    if (hasImageAttachments) {
      formattedContext += "\n注意：原对话包含图片，我已经提供了图片链接。通义千问支持图片，如果需要可以尝试手动上传相关图片。\n\n";
    }
  ...
  trySubmitForm(inputElement) {
    try {
      ...
      if (sendButton && !sendButton.disabled) {
        console.log('Clicking Tongyi send button');
        sendButton.click();
      } else {
